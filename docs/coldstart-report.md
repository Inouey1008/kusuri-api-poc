...
Lambda 上で Go + SQLite は動作した。ウォーム約 30ms、コールドスタート約 350ms で、いずれも API として実用に足る。一方でメモリサイズの結論は本番データでは覆る。検証に使った DB が 3 行しかなく、SQLite へのクエリを一度も計測していないため。
...
| 3 | コールドスタートは許容範囲か | △ | 約 350ms。許容値が未定のため判定保留 |
...
### 8.2 覆る: 「コールドスタート 350ms」

この値はパッケージ取得が 65% を占める。DB を同梱する方式では、添付文書の本文をどこまで持たせるかで zip サイズが桁で変わる。
...
3. **コールドスタートの許容値を決める** — 現状の 350ms が問題にならないなら、メモリを下げてコストを削る判断ができる
